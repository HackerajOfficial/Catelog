...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = "sqlite:///catelog.db"
...
class Product(db.Model):
    ...
    colors = db.relationship('Color', secondary=product_color_association, backref='products')
...
def createProduct():
    if request.method == 'POST':
        ...
        if cover:
            filename = secure_filename(cover.filename) + '_' + str(uuid.uuid4()) 
        ...
        return render_template('createProduct.html', colors=colors, sizes=sizes)
